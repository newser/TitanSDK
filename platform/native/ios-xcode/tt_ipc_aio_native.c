...
// send
static tt_bool_t __do_ipc_send(IN __ipc_send_t *aio, IN tt_result_t kq_result);
static tt_result_t __do_ipc_send_io(IN __ipc_send_t *aio);
static void __do_ipc_send_cb(IN __ipc_send_t *aio);

// recv
static tt_bool_t __do_ipc_recv(IN __ipc_recv_t *aio, IN tt_result_t kq_result);
...
conn_ag:
    ...
    if ((tt_list_count(&sys_ipc->write_q) == 1) &&
        !TT_OK(__ipc_kq_wr_enable(sys_ipc))) {
                                    ...
                                    IN OPT void *cb_param)
{
    ...
    if ((tt_list_count(&sys_ipc->read_q) == 1) &&
        !TT_OK(__ipc_kq_rd_enable(sys_ipc))) {
        tt_list_remove(&tev->node);
        tt_ev_destroy(ev);
        return TT_FAIL;
    }
...
tt_result_t tt_ipc_kq_rd_handler(IN struct tt_ipc_s *ipc,
                                 IN tt_u32_t flags,
                                 IN tt_u32_t fflags,
                                 IN tt_uintptr_t data)
{
    ...
    if (tt_list_empty(&sys_ipc->read_q)) {
        TT_WARN("EVFILT_READ, but no read aio");
    }

    // note the sequence of checking epoll event

    // 1. do io whenever epoll report socket is readable
    ...
    TT_WARN("missing event: %x", flags & ~(EVFILT_READ | EV_EOF | EV_ERROR));
    return TT_SUCCESS;
}

tt_result_t tt_ipc_kq_wr_handler(IN struct tt_ipc_s *ipc,
                                 IN tt_u32_t flags,
                                 IN tt_u32_t fflags,
                                 IN tt_uintptr_t data)
{
    ...
    if (tt_list_empty(&sys_ipc->write_q)) {
        TT_WARN("EVFILT_WRITE, but no write aio");
    }
    ...
    // 2. there are only error events
    if (flags & EV_ERROR) {
        __do_ipc_aio_q(sys_ipc, &sys_ipc->write_q, TT_FAIL);
        return TT_SUCCESS;
    }

    TT_WARN("missing event: %x", flags & ~(EVFILT_WRITE | EV_ERROR));
    return TT_SUCCESS;
}
                ...
                IN tt_ipc_exit_t *ipc_exit)
{
    ...
    sys_ipc->evc = evc;
    sys_ipc->ev_mark_rd = TT_IPC_MARK_KQ_RD;
    sys_ipc->ev_mark_wr = TT_IPC_MARK_KQ_WR;
                             ...
                             IN tt_evcenter_t *evc)
{
    ...
    if (local_addr != NULL) {
        ...
        unlink(local_addr);
        if (bind(ipc, (const struct sockaddr *)&skt_addr, skt_addr.sun_len) !=
            0) {
        ...
        // ipc with NULL local addr are either client ipc or accepted ipc
        if (!TT_OK(tt_buf_reserve(&sys_ipc->recv_buf, attr->recv_buf_size))) {
            TT_ERROR("fail to reserve [%d] bytes ipc recv buf",
                     attr->recv_buf_size);
            goto __cis_fail;
        }
    }
    ...
    if (setsockopt(ipc,
        ...
        goto __cis_fail;
#else
        TT_WARN("fail to set SO_NOSIGPIPE");
#endif
    }
    ...
    // add to kqueue
    if (!TT_OK(
            __ipc_kq_rd_add(evc->sys_evc.kq_fd, ipc, &sys_ipc->ev_mark_rd)) ||
        !TT_OK(
            __ipc_kq_wr_add(evc->sys_evc.kq_fd, ipc, &sys_ipc->ev_mark_wr))) {
                            ...
                            IN const tt_char_t *addr)
{
    ...
    // len
    skt_addr->sun_len =
        sizeof(struct sockaddr_un) - sizeof(skt_addr->sun_path) + addr_len;

    return TT_SUCCESS;
}

tt_bool_t __do_ipc_connect(IN __ipc_connect_t *aio, IN tt_result_t kq_result)
{
...
tt_bool_t __do_ipc_accept(IN __ipc_accept_t *aio, IN tt_result_t kq_result)
{
    ...
    accept_ag:
        ...
        if (new_sys_ipc->s == -1) {
            ...
            if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
                if (!TT_OK(__ipc_kq_rd_enable(listening_sys_ipc))) {
...
tt_bool_t __do_ipc_send(IN __ipc_send_t *aio, IN tt_result_t kq_result)
{
    if (TT_OK(kq_result)) {
    ...
    while (aio->send_len < size) {
        ...
        if (write_num > 0) {
            ...
            if (errno == EINTR) {
                ...
                if (TT_OK(__ipc_kq_wr_enable(sys_ipc))) {
...
tt_bool_t __do_ipc_recv(IN __ipc_recv_t *aio, IN tt_result_t kq_result)
{
    if (TT_OK(kq_result)) {
        // only do io when kq returned success
...
tt_result_t __do_ipc_recv_io(IN __ipc_recv_t *aio)
{
    ...
    while (TT_BUF_WLEN(recv_buf) > 0) {
        ...
        if (read_num > 0) {
            TT_ASSERT_ALWAYS(
                TT_OK(tt_buf_inc_wp(recv_buf, (tt_u32_t)read_num)));
            continue;
            // while loop would check if all are written
        } else if (read_num == 0) {
            return TT_END;
            ...
            if (errno == EINTR) {
                ...
                // doing io
                if (TT_OK(__ipc_kq_rd_enable(sys_ipc))) {
                    break;
                } else {
                    return TT_FAIL;
                }
                    ...
                    IN tt_result_t kq_result)
{
    ...
    while ((node = tt_list_head(aio_q)) != NULL) {
        ...
        switch (ev->ev_id) {
            case EV_IPC_ACCEPT: {
                ...
                aio_done = __do_ipc_accept(aio, kq_result);
            } break;
            case EV_IPC_CONNECT: {
                ...
                aio_done = __do_ipc_connect(aio, kq_result);
            } break;
            case EV_IPC_SEND: {
                __ipc_send_t *aio = TT_EV_DATA(ev, __ipc_send_t);
                ...
                aio_done = __do_ipc_send(aio, kq_result);
            ...
            case EV_IPC_RECV: {
                ...
                aio_done = __do_ipc_recv(aio, kq_result);
    ...
    if ((aio_q == &sys_ipc->read_q) && (sys_ipc->s != -1)) {
        __ipc_kq_rd_disable(sys_ipc);
    }
    if ((aio_q == &sys_ipc->write_q) && (sys_ipc->s != -1)) {
        __ipc_kq_wr_disable(sys_ipc);
    }
}
