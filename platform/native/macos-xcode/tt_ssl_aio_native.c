...
{
    CFStringRef cn = NULL;
    CFStringRef cert_cn = NULL;
    OSStatus osst;
    tt_result_t result = TT_FAIL;

    cn = CFStringCreateWithCString(NULL, common_name, kCFStringEncodingUTF8);
    if (cn == NULL) {
        TT_ERROR("fail to create cn");
        goto __vfcn_out;
    }

    osst = SecCertificateCopyCommonName(cert, &cert_cn);
    if ((osst != noErr) || (cert_cn == NULL)) {
        TT_ERROR("fail to get cert common name");
        goto __vfcn_out;
    }

    if (CFStringCompare(cn, cert_cn, 0) == kCFCompareEqualTo) {
        result = TT_SUCCESS;
    }
