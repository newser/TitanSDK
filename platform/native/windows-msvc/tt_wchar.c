                         ...
                         OUT OPT tt_u32_t *wchar_bytes)
{
    ...
    char_num = MultiByteToWideChar(CP_UTF8, 0, utf8_str, -1, NULL, 0);
    if (char_num == 0) {
    ...
    char_num =
        MultiByteToWideChar(CP_UTF8, 0, utf8_str, -1, wchar_str, char_num);
    if (char_num == 0) {
...
tt_char_t *tt_utf8_create(IN wchar_t *wchar_str, OUT OPT tt_u32_t *utf8_bytes)
{
    ...
    // the length returned by the function includes this character.
    char_num =
        WideCharToMultiByte(CP_UTF8, 0, wchar_str, -1, NULL, 0, NULL, NULL);
    if (char_num == 0) {
