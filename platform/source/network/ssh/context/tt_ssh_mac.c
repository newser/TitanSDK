...
tt_result_t tt_sshmac_setalg(IN tt_sshmac_t *mac,
...
tt_result_t tt_sshmac_sign(IN tt_sshmac_t *mac,
                           ...
                           IN tt_u32_t signature_len)
{
    ...
    TT_ASSERT_SSH(TT_SSH_MAC_ALG_VALID(mac->alg));
    return tt_s_sshmac_itf[mac->alg]
        .sign(mac, seq_number, data, data_len, signature, signature_len);
}

tt_result_t tt_sshmac_verify(IN tt_sshmac_t *mac,
                             IN tt_u32_t seq_number,
                             IN tt_u8_t *data,
                             IN tt_u32_t data_len,
                             ...
                             IN tt_u32_t signature_len)
{
    ...
    TT_ASSERT_SSH(TT_SSH_MAC_ALG_VALID(mac->alg));
    return tt_s_sshmac_itf[mac->alg]
        .verify(mac, seq_number, data, data_len, signature, signature_len);
}
