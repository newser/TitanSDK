...
static tt_u32_t __can_send_len(IN tt_u32_t head_mac_size,
                               IN tt_u32_t block_size, IN tt_u32_t win_size,
                               IN tt_u32_t data_len);
static void __check_can_send(IN tt_sshch_t *ch);

static void __sshch_pkt_winadj(IN tt_sshch_t *ch, IN tt_sshmsg_chwinadj_t *chwa,
                               OUT struct tt_sshsvr_action_s *svract);
static void __sshch_pkt_chdata(IN tt_sshch_t *ch, IN tt_sshmsg_chdata_t *chdata,
                               OUT struct tt_sshsvr_action_s *svract);
static void __sshch_pkt_chreq(IN tt_sshch_t *ch, IN tt_sshmsg_chreq_t *chreq,
                              OUT struct tt_sshsvr_action_s *svract);

static tt_result_t __send_chdata(IN tt_sshch_t *ch, IN tt_u8_t *data,
                                 IN tt_u32_t data_len, IN tt_bool_t no_copy);
...
tt_sshch_t *tt_sshch_create(IN tt_ssh_chtype_t type, IN tt_u32_t chnum,
                            ...
                            IN struct tt_sshchmgr_s *chmgr)
{
    ...
    ch->chmgr = chmgr;
    if (!TT_OK(tt_sshchmgr_extra_size(chmgr, &ch->head_mac_size,
                                      &ch->block_size))) {
...
void tt_sshch_shutdown(IN tt_sshch_t *ch)
{
    ...
    // destroy
    if (ch->close_in && ch->close_out) { tt_sshch_destroy(ch); }
}

tt_result_t tt_sshch_send(IN tt_sshch_t *ch, IN tt_u8_t *data,
                          IN tt_u32_t data_len, IN tt_u32_t flag)
{
    ...
    if (data_len == 0) { return TT_SUCCESS; }
    TT_ASSERT(data != NULL);

    n = __can_send(ch, data_len);
    TT_ASSERT(n <= data_len);
    if (n == 0) { return tt_buf_put(&ch->send_buf, data, data_len); }

    if (!TT_OK(__send_chdata(ch, data, data_len, TT_FALSE))) { return TT_FAIL; }
...
void tt_sshch_pkt_handler(IN tt_sshch_t *ch, IN tt_sshmsg_t *msg,
                          OUT struct tt_sshsvr_action_s *svract)
{
    switch (msg->msg_id) {
    case TT_SSH_MSGID_CHANNEL_REQUEST: {
        __sshch_pkt_chreq(ch, TT_SSHMSG_CAST(msg, tt_sshmsg_chreq_t), svract);
        return;
    } break;
    case TT_SSH_MSGID_CHANNEL_DATA: {
        __sshch_pkt_chdata(ch, TT_SSHMSG_CAST(msg, tt_sshmsg_chdata_t), svract);
        return;
    } break;
    case TT_SSH_MSGID_CHANNEL_WINDOW_ADJUST: {
        __sshch_pkt_winadj(ch, TT_SSHMSG_CAST(msg, tt_sshmsg_chwinadj_t),
                           svract);
        return;
    } break;
...
{
    if (TT_BUF_RLEN(&ch->send_buf) != 0) { return 0; }

    return __can_send_len(ch->head_mac_size, ch->block_size,
                          TT_MIN(ch->pkt_size, ch->win_size), data_len);
}

tt_u32_t __can_send_len(IN tt_u32_t head_mac_size, IN tt_u32_t block_size,
                        IN tt_u32_t win_size, IN tt_u32_t data_len)
{
    tt_u32_t n;
...
void __check_can_send(IN tt_sshch_t *ch)
{
    ...
    len = __can_send_len(ch->head_mac_size, ch->block_size,
                         TT_MIN(ch->pkt_size, ch->win_size), len);
    if (len == 0) { return; }
...
void __sshch_pkt_winadj(IN tt_sshch_t *ch, IN tt_sshmsg_chwinadj_t *chwa,
                        OUT struct tt_sshsvr_action_s *svract)
{
...
void __sshch_pkt_chdata(IN tt_sshch_t *ch, IN tt_sshmsg_chdata_t *chdata,
                        OUT struct tt_sshsvr_action_s *svract)
{
    tt_buf_t *data = &chdata->data;
    tt_u32_t data_len = TT_BUF_RLEN(data);

    if (data_len > 0) { ch->cb->on_recv(ch, TT_BUF_RPOS(data), data_len); }
}

void __sshch_pkt_chreq(IN tt_sshch_t *ch, IN tt_sshmsg_chreq_t *chreq,
                       OUT struct tt_sshsvr_action_s *svract)
{
    tt_result_t result;
    ...
    if (chreq->want_reply) {
        if (TT_OK(result)) { __send_chsucc(ch, chreq->rcv_chnum); }
    }
}

tt_result_t __send_chdata(IN tt_sshch_t *ch, IN tt_u8_t *data,
                          IN tt_u32_t data_len, IN tt_bool_t no_copy)
{
    tt_sshmsg_t *chdata;

    chdata = tt_sshmsg_chdata_create();
    if (chdata == NULL) { return TT_FAIL; }
