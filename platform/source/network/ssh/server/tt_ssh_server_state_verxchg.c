                                 ...
                                 IN tt_sshmsg_t *msg)
{
    ...
    case TT_SSH_MSGID_VERXCHG: {
        ...
        if (!TT_OK(tt_sshctx_kex_setvc(&svrconn->ctx, TT_BUF_RPOS(&msg->buf),
                                       TT_BUF_RLEN(&msg->buf) - 2, TT_TRUE))) {
            return TT_FAIL;
        }
...
tt_result_t __svr_verxchg_verxchg(IN tt_sshsvrconn_t *svrconn, IN void *param)
{
    ...
    if (!TT_OK(tt_sshms_verxchg_set_swver(msg, "TitanSDK"))) { return TT_FAIL; }

    // need record vs
    if (!TT_OK(tt_sshmsg_render(msg, svrconn->ctx.encrypt.block_len, NULL))) {
        return TT_FAIL;
    }
    if (!TT_OK(tt_sshctx_kex_setvs(&svrconn->ctx, TT_BUF_RPOS(&msg->buf),
                                   TT_BUF_RLEN(&msg->buf) - 2, TT_TRUE))) {
        return TT_FAIL;
    }
