    ...
    TT_TEST_ROUTINE_DEFINE(tt_adns_ut_rr_basic)
{
    ...
    rr = tt_adns_rr_create(0,
                           ...
                           TT_ADNS_RR_A_IN,
                           100);
    TT_UT_NOT_EQUAL(rr, NULL, "");
    TT_UT_EQUAL(strcmp(rr->name, "example.com"), 0, "");
    TT_UT_EQUAL(rr->name_len, strlen("example.com") + 1, "");
    TT_UT_EQUAL(rr->type, TT_ADNS_RR_A_IN, "");
    TT_UT_EQUAL(rr->ttl, 100, "");

    rr1 = tt_adns_rr_copy(rr);
    TT_UT_NOT_EQUAL(rr1, NULL, "");
    TT_UT_NOT_EQUAL(rr1->name, rr->name, "");
    TT_UT_EQUAL(strcmp(rr1->name, "example.com"), 0, "");
    TT_UT_EQUAL(rr1->name_len, strlen("example.com") + 1, "");
    ...
    // change name
    TT_UT_EQUAL(tt_adns_rr_set_name(rr1, "aaa", 0, TT_ADNS_RR_COPY_NAME),
                TT_SUCCESS,
                "");
    TT_UT_EQUAL(rr1->name_len, 4, "");
    TT_UT_EQUAL(strcmp(rr1->name, "aaa"), 0, "");
    TT_UT_EQUAL(rr1->name_owner, TT_TRUE, "");

    // change name
    TT_UT_EQUAL(tt_adns_rr_set_name(rr1,
                                    "example.com",
                                    12,
                                    TT_ADNS_RR_REF_NAME),
                TT_SUCCESS,
                "");
    TT_UT_EQUAL(rr1->name_len, 12, "");
    TT_UT_EQUAL(strcmp(rr1->name, "example.com"), 0, "");
    TT_UT_EQUAL(rr1->name_owner, TT_FALSE, "");

    // change name
    do {
        tt_char_t *nn = tt_malloc(sizeof("12345"));
        TT_UT_NOT_EQUAL(nn, NULL, "");

        tt_memcpy(nn, "12345", sizeof("12345"));
        TT_UT_EQUAL(tt_adns_rr_set_name(rr1, nn, 6, TT_ADNS_RR_TAKE_NAME),
                    TT_SUCCESS,
                    "");
        TT_UT_EQUAL(rr1->name_len, 6, "");
        TT_UT_EQUAL(strcmp(rr1->name, "12345"), 0, "");
        TT_UT_EQUAL(rr1->name_owner, TT_TRUE, "");
    } while (0);
    ...
    do {
        ...
        while (node != NULL) {
            ...
            TT_UT_EQUAL(strcmp(__rr2->name, "example.com"), 0, "");
            TT_UT_EQUAL(__rr2->name_len, strlen("example.com") + 1, "");
...
TT_TEST_ROUTINE_DEFINE(tt_adns_ut_rr_question)
{
    ...
    TT_UT_NOT_EQUAL(rr, NULL, "");
    TT_UT_EQUAL(rr->is_quest, TT_TRUE, "");

    // render

    ret = tt_adns_quest_render_prepare(rr, &len);
    TT_UT_EQUAL(ret, TT_SUCCESS, "");
    TT_UT_EQUAL(len, sizeof(raw_data) - 1, "");

    expect_len = len;
    tt_buf_create(&ds, 0, NULL);
    st = tt_adns_quest_render(rr, &ds);
    TT_UT_EQUAL(st, TT_SUCCESS, "");
    TT_UT_EQUAL(TT_BUF_RLEN(&ds), expect_len, "");
    TT_UT_EQUAL(tt_memcmp(ds.p, raw_data, expect_len), 0, "");
    ...
    TT_UT_EQUAL(rr->is_quest, TT_TRUE, "");
    TT_UT_EQUAL(rr->name_len, 4, "");
    TT_UT_EQUAL(tt_memcmp(rr->name, "aaa", 4), 0, "");
    TT_UT_EQUAL(rr->type, TT_ADNS_RR_A_IN, "");
...
TT_TEST_ROUTINE_DEFINE(tt_adns_ut_rr_a_in)
{
    ...
    TT_UT_EQUAL(tt_memcmp(rr->name, "aaa", 4), 0, "");
    TT_UT_EQUAL(rr->type, TT_ADNS_RR_A_IN, "");
    TT_UT_EQUAL(rr->ttl, 10, "");
    TT_UT_EQUAL(rr->rdlength, 4, "");
    TT_UT_EQUAL(tt_memcmp(&(TT_ADRR_CAST(rr, tt_adrr_a_t)->addr),
                          &rdata.addr,
                          4),
                0,
                "");
...
TT_TEST_ROUTINE_DEFINE(tt_adns_ut_rr_aaaa_in)
{
    ...
    TT_UT_EQUAL(rr->name_len, 9, "");
    TT_UT_EQUAL(tt_memcmp(rr->name, "ipv6.com", 9), 0, "");
    TT_UT_EQUAL(rr->type, TT_ADNS_RR_AAAA_IN, "");
    TT_UT_EQUAL(rr->ttl, 0x1020304, "");
    TT_UT_EQUAL(rr->rdlength, sizeof(tt_adrr_aaaa_t), "");
    TT_UT_EQUAL(tt_memcmp(&TT_ADRR_CAST(rr, tt_adrr_aaaa_t)->addr,
                          &rdata.addr,
                          16),
                0,
                "");
...
TT_TEST_ROUTINE_DEFINE(tt_adns_ut_rr_cname_in)
{
    ...
    TT_UT_EQUAL(rr->name_len, 14, "");
    TT_UT_EQUAL(tt_memcmp(rr->name, "cname.example", 14), 0, "");
    TT_UT_EQUAL(rr->type, TT_ADNS_RR_CNAME_IN, "");
    TT_UT_EQUAL(rr->ttl, 10, "");
    {
        tt_adrr_cname_t *rdd = TT_ADRR_CAST(rr, tt_adrr_cname_t);
        TT_UT_EQUAL(rr->rdlength, 7, "");
        TT_UT_EQUAL(strcmp(rdd->cname, rdata.cname), 0, "");
        TT_UT_EQUAL(rdd->cname_len, strlen(rdd->cname) + 1, "");
    }
...
__parse : {
...

    {
        ...
        st = tt_adns_pkt_parse_prepare(&ds1);
        TT_UT_EQUAL(st, TT_BUFFER_INCOMPLETE, "");
        tt_buf_destroy(&ds1);
    }

    tt_buf_create_nocopy(&ds, pos, len, NULL);
    pkt = tt_adns_pkt_parse(&ds, TT_ADNS_PKT_PARSE_ALL);
    TT_UT_NOT_EQUAL(pkt, NULL, "");

    TT_UT_EQUAL(pkt->__id, 0x743a, "");
    TT_UT_EQUAL(pkt->opcode, 0, "");
    TT_UT_NOT_EQUAL(pkt->flag & __ADNS_PKT_RESP, 0, "");
    TT_UT_EQUAL(pkt->flag & __ADNS_PKT_AUTH_ANS, 0, "");
    TT_UT_NOT_EQUAL(pkt->flag & __ADNS_PKT_RECUR_DESIRED, 0, "");
    TT_UT_NOT_EQUAL(pkt->flag & __ADNS_PKT_RECUR_AVAIL, 0, "");
    TT_UT_EQUAL(pkt->rcode, 0, "");

    TT_UT_EQUAL(pkt->question_num, 1, "");
    TT_UT_EQUAL(pkt->answer_num, 2, "");
    TT_UT_EQUAL(pkt->authority_num, 0, ""); // it does not know NS rr
    TT_UT_EQUAL(pkt->additional_num, 12, "");
    ...
    while (node != NULL) {
        ...
        if (rr->type == TT_ADNS_RR_A_IN) {
            TT_UT_EQUAL(strcmp(rr->name,
                               "b523704d001c67bf765a54fc0529a3ef.7cname.com"),
                        0,
                        "");
            TT_UT_EQUAL(strlen("b523704d001c67bf765a54fc0529a3ef.7cname.com") +
                            1,
                        rr->name_len,
                        "");

            TT_UT_EQUAL(rr->ttl, 0x1c0, "");
            TT_UT_EQUAL(TT_ADRR_CAST(rr, tt_adrr_a_t)->addr.a32.__u32,
                        tt_ntoh32(0x790c7c66),
                        "");

            status |= 1;
        } else if (rr->type == TT_ADNS_RR_CNAME_IN) {
            TT_UT_EQUAL(strcmp(rr->name, "recdm.csdn.net"), 0, "");
            TT_UT_EQUAL(strlen("recdm.csdn.net") + 1, rr->name_len, "");

            TT_UT_EQUAL(rr->ttl, 0x1c0, "");
            TT_UT_EQUAL(strcmp(TT_ADRR_CAST(rr, tt_adrr_cname_t)->cname,
                               "b523704d001c67bf765a54fc0529a3ef.7cname.com"),
                        0,
                        "");
            TT_UT_EQUAL(strlen("b523704d001c67bf765a54fc0529a3ef.7cname.com") +
    ...
    TT_UT_EQUAL(status, 3, "");

    // render
    if (!rendered) {
        TT_UT_EQUAL(tt_adns_pkt_generate(pkt), TT_SUCCESS, "");
