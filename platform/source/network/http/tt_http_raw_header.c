...
tt_http_rawval_t *tt_http_rawval_create(IN tt_slab_t *slab, IN tt_char_t *val,
                                        ...
                                        IN tt_u32_t len)
{
    ...
    while (node != NULL) {
        ...
        if (tt_blobex_memcmp(&rv->val, (tt_u8_t *)val, len) == 0) { return rv; }
    }
    return NULL;
}

tt_result_t tt_http_rawval_add_n(IN tt_slab_t *slab, IN tt_dlist_t *dl,
                                 IN const tt_char_t *val, IN tt_u32_t len)
{
    tt_http_rawval_t *rv =
        tt_http_rawval_create(slab, (tt_char_t *)val, len, TT_TRUE);
    if (rv == NULL) { return TT_FAIL; }

    tt_dlist_push_tail(dl, &rv->dnode);
    return TT_SUCCESS;
}

tt_http_rawhdr_t *tt_http_rawhdr_create(IN tt_slab_t *slab, IN tt_char_t *name,
                                        IN tt_u32_t len, IN tt_bool_t owner)
{
...
tt_u32_t tt_http_rawhdr_count_name_n(IN tt_dlist_t *dl, IN tt_char_t *name,
                                     IN tt_u32_t len)
{
    tt_dnode_t *node = tt_dlist_head(dl);
    tt_u32_t count = 0;
...
tt_http_rawhdr_t *__rawhdr_find(IN tt_dnode_t *node, IN const tt_char_t *name,
                                IN tt_u32_t len)
{
    while (node != NULL) {
        tt_http_rawhdr_t *rh = TT_CONTAINER(node, tt_http_rawhdr_t, dnode);
