...
{
    tt_u8_t aligned_key[__AES_MAX_KEY_SIZE], aligned_iv[TT_AES_BLOCK_SIZE];
    ...
    if (size == TT_AES_SIZE_128) {
        ...
        aligned_key_len = TT_AES128_KEY_SIZE;
    } else {
        TT_ASSERT(size == TT_AES_SIZE_256);
        if (key->len > TT_AES256_KEY_SIZE) {
        ...
        aligned_key_len = TT_AES256_KEY_SIZE;
    }
