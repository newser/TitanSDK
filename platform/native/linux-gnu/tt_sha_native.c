...
typedef \
tt_result_t (*__sha_update_t)(IN tt_sha_ntv_t *sys_sha,
                              IN tt_u8_t *input,
                              IN tt_u32_t input_len);
typedef \
tt_result_t (*__sha_final_t)(IN tt_sha_ntv_t *sys_sha, OUT tt_u8_t *output);
...
tt_result_t __sha1_update(IN tt_sha_ntv_t *sys_sha,
                          ...
                          IN tt_u32_t input_len)
{
    SHA1_Update(&sys_sha->sha1, input, input_len);
    return TT_SUCCESS;
}

tt_result_t __sha1_final(IN tt_sha_ntv_t *sys_sha, OUT tt_u8_t *output)
{
    SHA1_Final(output, &sys_sha->sha1);
    return TT_SUCCESS;
}
