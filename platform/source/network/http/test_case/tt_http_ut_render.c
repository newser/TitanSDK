...
TT_TEST_CASE("case_http_render_req", "http render req", case_http_render_req,
             NULL, NULL, NULL, NULL, NULL)
,

    TT_TEST_CASE("case_http_render_resp", "http render resp",
                 case_http_render_resp, NULL, NULL, NULL, NULL, NULL),

    TT_TEST_CASE("case_http_svcmgr", "http service manager", case_http_svcmgr,
                 NULL, NULL, NULL, NULL, NULL),

    TT_TEST_CASE("case_http_svcmgr_encserv", "http service mgr encoding serv",
                 case_http_svcmgr_encserv, NULL, NULL, NULL, NULL, NULL),
    ...
    // 2 headers
    {
...

        {
            ...
            tt_http_txenc_t txe[5] = {
                TT_HTTP_TXENC_GZIP, TT_HTTP_TXENC_DEFLATE,
                TT_HTTP_TXENC_GZIP, TT_HTTP_TXENC_CHUNKED,
                TT_HTTP_TXENC_GZIP,
            };
            tt_http_enc_t cte[2] = {
                TT_HTTP_ENC_COMPRESS,
                TT_HTTP_ENC_COMPRESS,
            };
            ...
            {
                TT_UT_SUCCESS(tt_http_req_render_add_ifmatch(&r, "123",
                                                             TT_FALSE),
                              "");
                TT_UT_SUCCESS(tt_http_req_render_add_ifmatch(&r, "456",
                                                             TT_TRUE),
                              "");
                TT_UT_SUCCESS(tt_http_req_render_add_ifmatch_aster(&r), "");
            }
            {
                TT_UT_SUCCESS(tt_http_req_render_add_ifnmatch(&r, "123",
                                                              TT_FALSE),
                              "");
                TT_UT_SUCCESS(tt_http_req_render_add_ifnmatch(&r, "456",
                                                              TT_TRUE),
                              "");
...
#define add_dyn_inserv()                                                       \
    do {                                                                       \
        for (i = 0; i < __IS_NUM; ++i) {                                       \
            ...
            ii->idx = __IS_NUM + i;                                            \
                                                                               \
            tt_http_svcmgr_add_dynamic_inserv(&sm, is[__IS_NUM + i],           \
                                              (void *)(tt_uintptr_t)(          \
                                                  __IS_NUM + i),               \
                                              &ctx_created[i]);                \
                                                                               \
...
TT_TEST_ROUTINE_DEFINE(case_http_svcmgr)
{
    ...
    tt_http_inserv_itf_t is_itf = {__inserv_destroy, __inserv_clear,
                                   __inserv_create_ctx, __inserv_destroy_ctx,
                                   __inserv_clear_ctx};
    tt_http_inserv_cb_t is_cb = {
        __is_on_uri,      __is_on_header,   __is_on_body,
        __is_on_trailing, __is_on_complete, __is_get_body,
    };
...

    {
        ...
        for (i = 0; i < __IS_NUM; ++i) { TT_UT_TRUE(__is_clear[i], ""); }
        for (i = 0; i < __IS_NUM; ++i) {
        ...
        // on header
        {
            ...
            // on trailing
            tt_memset(__is_on_trailing_called, 0,
            ...
            // on complete
            tt_memset(__is_on_complete_called, 0,
        ...
        tt_http_svcmgr_on_header(&sm, NULL, NULL);
        {
            ...
            TT_UT_EQUAL(tt_http_svcmgr_on_resp_header(&sm, NULL, NULL),
                        TT_E_END, "");
            for (i = 0; i <= 1; ++i) { TT_UT_TRUE(__os_on_resp[i], ""); }
            for (; i < __OS_NUM; ++i) { TT_UT_FALSE(__os_on_resp[i], ""); }

            tt_memset(__os_clear, 0, sizeof(__os_clear));
            tt_http_svcmgr_clear(&sm);
            for (i = 0; i < __OS_NUM; ++i) { TT_UT_TRUE(__os_clear[i], ""); }
        }
...
TT_TEST_ROUTINE_DEFINE(case_http_svcmgr_encserv)
{
    ...
    tt_buf_t *pbuf;
    tt_buf_t ibuf;
    tt_http_txenc_t txe[] = {TT_HTTP_TXENC_DEFLATE, TT_HTTP_TXENC_GZIP,
                             TT_HTTP_TXENC_CHUNKED};
...

    {
        ...
        for (i = 0; i < __ES_NUM; ++i) { TT_UT_TRUE(__es_pre_bodyed[i], ""); }

        ret = tt_http_svcmgr_on_resp_body(&sm, NULL, &resp, &ibuf, &pbuf);
        TT_UT_SUCCESS(ret, "");
        TT_UT_EQUAL(pbuf, &__es_on_buf[1], "");
        for (i = 0; i < __ES_NUM; ++i) { TT_UT_TRUE(__es_on_bodyed[i], ""); }

        ret = tt_http_svcmgr_post_body(&sm, NULL, &resp, &ibuf, &pbuf);
        TT_UT_SUCCESS(ret, "");
        TT_UT_EQUAL(pbuf, &__es_post_buf[1], "");
        for (i = 0; i < __ES_NUM; ++i) { TT_UT_TRUE(__es_post_bodyed[i], ""); }

        tt_http_svcmgr_clear(&sm);
        for (i = 0; i < __ES_NUM; ++i) { TT_UT_TRUE(__es_cleared[i], ""); }
    ...
    tt_http_svcmgr_destroy(&sm);
    for (i = 0; i < __ES_NUM; ++i) { TT_UT_TRUE(__es_destroyed[i], ""); }
