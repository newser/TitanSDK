...
extern tt_result_t __ssl_destroy_aio(IN tt_ev_t *tev);
...
// send
static tt_bool_t __do_skt_send(IN __skt_send_t *aio, IN tt_result_t ep_result);
...
// recv
static tt_bool_t __do_skt_recv(IN __skt_recv_t *aio, IN tt_result_t ep_result);
...
tt_result_t __skt_start_epoll(IN tt_skt_ntv_t *sys_skt);
                               ...
                               IN tt_u32_t flag)
{
    ...
    if (flag & __ASC_ACCEPT) {
        ...
        do {
            ...
            ep_ev.data.ptr = &sys_skt->ev_mark;

            if (epoll_ctl(evc->sys_evc.ep_fd, EPOLL_CTL_ADD, __skt, &ep_ev) !=
                0) {
    ...
    sys_skt->s = __skt;
    sys_skt->evc = evc;
    sys_skt->ev_mark = TT_EVP_MARK_EPOLL_SOCKET;
    sys_skt->role = role;
                                    ...
                                    IN OPT void *cb_param)
{
    ...
    // start epoll if it's head aio
    if ((tt_list_count(&sys_skt->read_q) == 1) &&
        !TT_OK(__skt_start_epoll(sys_skt))) {
                                  ...
                                  IN OPT void *cb_param)
{
    ...
    if (tt_list_count(&sys_skt->read_q) == 1) {
        ...
        if ((sys_skt->ssl != NULL) &&
            (SSL_pending(sys_skt->ssl->sys_ssl.ssl))) {
            // if there are buffered data in ssl, we should
            // read it from ssl rather than polling socket.
        ...
        if (!TT_OK(__skt_start_epoll(sys_skt))) {
            tt_list_remove(&tev->node);
            tt_ev_destroy(ev);
            return TT_FAIL;
        }
...
tt_result_t tt_skt_ep_handler(IN tt_skt_t *skt, IN tt_u32_t ep_events)
{
    ...
    if (ep_events & EPOLLRDHUP) {
        ...
        //__do_skt_aio_q(sys_skt, &sys_skt->write_q, TT_END);
        return TT_SUCCESS;
    }
...
tssa_fail:
    ...
    if (skt->sys_socket.s != -1) {
        __RETRY_IF_EINTR(close(skt->sys_socket.s) != 0);
        skt->sys_socket.s = -1;
    }
...
#ifdef TT_PLATFORM_SSL_ENABLE
    if (sys_skt->ssl != NULL) {
        if (__ssl_close_notify(sys_skt->ssl) == TT_PROCEEDING) {
            // wait until close_notify could be sent
            __skt_start_epoll(sys_skt);
            return TT_FALSE;
        }
        // continue even if there is an ssl error
    }
#endif
...
tt_bool_t __do_skt_accept(IN __skt_accept_t *aio, IN tt_result_t ep_result)
{
    ...
    if (TT_OK(ep_result)) {
        ...
        ep_ev.events = EPOLLRDHUP;
        ep_ev.data.ptr = &new_sys_skt->ev_mark;

    accept_ag:
#ifndef __SKT_AIO_ACCEPT4
        new_sys_skt->s = accept(listening_sys_skt->s,
                                (struct sockaddr *)&remote_addr,
                                &remote_addr_len);
#else
        new_sys_skt->s = accept4(listening_sys_skt->s,
                                 (struct sockaddr *)&remote_addr,
                                 &remote_addr_len,
                                 SOCK_NONBLOCK);
#endif
        if (new_sys_skt->s == -1) {
            ...
            if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
                if (!TT_OK(__skt_start_epoll(listening_sys_skt))) {
            ...
            aio->result = TT_FAIL;
        } else if (epoll_ctl(sys_evc->ep_fd,
                             EPOLL_CTL_ADD,
                             new_sys_skt->s,
                             &ep_ev) != 0) {
            TT_ERROR_NTV("fail to add to ep");
...
tt_bool_t __do_skt_connect(IN __skt_connect_t *aio, IN tt_result_t ep_result)
{
...
tt_bool_t __do_skt_send(IN __skt_send_t *aio, IN tt_result_t ep_result)
{
    if (TT_OK(ep_result)) {
#ifdef TT_PLATFORM_SSL_ENABLE
...
tt_result_t __do_skt_send_io(IN __skt_send_t *aio)
{
    ...
    while (sbuf_state->cur_idx < sbuf_state->iov_num) {
        ...
        if (write_num > 0) {
            __update_sbuf_state(sbuf_state, write_num);
            continue;
...
tt_result_t __do_skt_send_io_ssl(IN __skt_send_t *aio)
{
    tt_skt_ntv_t *sys_skt = &aio->skt->sys_socket;
    __sbuf_state_t *sbuf_state = &aio->sbuf_state;
    SSL *ssl = sys_skt->ssl->sys_ssl.ssl;
    ...
    while (sbuf_state->cur_idx < sbuf_state->iov_num) {
        ...
        len = cur_v->iov_len - sbuf_state->cur_pos;

        ret = SSL_write(ssl, pos, len);
        if (ret > 0) {
            __update_sbuf_state(sbuf_state, ret);
            continue;
            // while loop would check if all are written
        } else {
            ...
            switch (err) {
                case SSL_ERROR_ZERO_RETURN: {
                    // ssl close notify or tcp shutdown
                    return TT_END;
                } break;
#if 0 // re-negotiation is not supported
...
tt_bool_t __do_skt_recv(IN __skt_recv_t *aio, IN tt_result_t ep_result)
{
    if (TT_OK(ep_result)) {
#ifdef TT_PLATFORM_SSL_ENABLE
        if (aio->skt->sys_socket.ssl != NULL) {
...
tt_result_t __do_skt_recv_io(IN __skt_recv_t *aio)
{
    ...
    while (sbuf_state->cur_idx < sbuf_state->iov_num) {
        ...
        if (read_num > 0) {
            __update_sbuf_state(sbuf_state, read_num);
            continue;
            // while loop would check if all are written
        } else if (read_num == 0) {
            return TT_END;
    ...
    while (sbuf_state->cur_idx < sbuf_state->iov_num) {
        ...
        if (ret > 0) {
            ...
            switch (err) {
                ...
                case SSL_ERROR_WANT_READ: {
                    if (sbuf_state->io_bytes > 0) {
                        // let caller know data received, despite
                        // whether data buffer is full
                        return TT_SUCCESS;
                    }
...
tt_bool_t __do_skt_sendto(IN __skt_sendto_t *aio, IN tt_result_t ep_result)
{
    if (TT_OK(ep_result)) {
...
tt_bool_t __do_skt_recvfrom(IN __skt_recvfrom_t *aio, IN tt_result_t ep_result)
{
    if (TT_OK(ep_result)) {
                    ...
                    IN tt_result_t ep_result)
{
    ...
    if (sys_skt->ssl_rd_handshaking || sys_skt->ssl_wr_handshaking) {
        tt_result_t hsk_result = __ssl_handshake(sys_skt->ssl, ep_result);
        if (TT_OK(hsk_result)) {
            // some aios may have been posted, but not start epoll
            // as ssl_connect or ssl_accept is the head aio when
            // those aios are posted
            //__skt_start_epoll(sys_skt);
    ...
    while ((node = tt_list_head(aio_q)) != NULL) {
        ...
        switch (ev->ev_id) {
            case EV_SKT_SHUTDOWN_WRITE: {
                ...
                tt_list_remove(&tev->node);
                aio_done = __do_skt_shutdown_wr(aio, ep_result);
                if (aio_done) {
                    // - during shutdown write, ssl may need send out close
                    // notify
                    //   so it may need keep waiting
                    // - if aio is done, the skt may already been freed, so do
                    // not
                    //   access the skt and destroy the aio here and return
                    tt_ev_destroy(ev);
                } else {
                    TT_ASSERT_SKTAIO(
                        tt_list_count(&aio->skt->sys_socket.write_q) == 0);
                    tt_list_addtail(&aio->skt->sys_socket.write_q, &tev->node);
                }
                return;
            } break;
            case EV_SKT_SHUTDOWN_READ: {
                ...
                // __do_skt_shutdown_wr may destroy socket
                tt_list_remove(&tev->node);
                __do_skt_shutdown_rd(aio, ep_result);
            ...
            case EV_SKT_ACCEPT: {
                ...
                aio_done = __do_skt_accept(aio, ep_result);
            } break;
            case EV_SKT_CONNECT: {
                ...
                aio_done = __do_skt_connect(aio, ep_result);
            } break;
            case EV_SKT_SEND: {
                __skt_send_t *aio = TT_EV_DATA(ev, __skt_send_t);
                ...
                aio_done = __do_skt_send(aio, ep_result);
            } break;
            case EV_SKT_RECV: {
                __skt_recv_t *aio = TT_EV_DATA(ev, __skt_recv_t);
                ...
                aio_done = __do_skt_recv(aio, ep_result);
            } break;
            case EV_SKT_SENDTO: {
                __skt_sendto_t *aio = TT_EV_DATA(ev, __skt_sendto_t);
                ...
                aio_done = __do_skt_sendto(aio, ep_result);
            } break;
            case EV_SKT_RECVFROM: {
                ...
                aio_done = __do_skt_recvfrom(aio, ep_result);
            } break;
    ...
    // - when ssl handhshake done, the socket is invalid and no
                              ...
                              IN tt_u32_t io_bytes)
{
    ...
    while (sbuf_state->cur_idx < sbuf_state->iov_num) {
        ...
        TT_ASSERT_SKTAIO(sbuf_state->cur_pos < cur_v->iov_len);
        left_space = cur_v->iov_len - sbuf_state->cur_pos;
...
tt_result_t __skt_start_epoll(IN tt_skt_ntv_t *sys_skt)
{
    ...
    tt_evcenter_ntv_t *sys_evc = &sys_skt->evc->sys_evc;
    int ep_fd = sys_evc->ep_fd;
    struct epoll_event ep_ev = {0};

    // event
    if (!tt_list_empty(&sys_skt->read_q)) {
        if (!sys_skt->ssl_rd_handshaking || sys_skt->ssl_want_rd) {
            // - not handshaking
            // - in handshaking and want to read
            ep_ev.events |= (EPOLLIN | EPOLLRDHUP);
        }
    }
    if (!tt_list_empty(&sys_skt->write_q)) {
        if (!sys_skt->ssl_wr_handshaking || sys_skt->ssl_want_wr) {
            // - not handshaking
            // - in handshaking and want to write
            ep_ev.events |= EPOLLOUT;
        }
    }

    ep_ev.data.ptr = &skt->sys_socket.ev_mark;

#if 0
    TT_INFO("skt[%d] ep[%d] events: %s, %s, %s",
            sys_skt->s,
            ep_fd,
    ...
    // start epoll
    if (epoll_ctl(ep_fd, EPOLL_CTL_MOD, sys_skt->s, &ep_ev) != 0) {
        TT_ERROR_NTV("fail to start epoll read");
        return TT_FAIL;
    }
