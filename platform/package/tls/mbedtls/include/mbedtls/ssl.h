...
#define MBEDTLS_ERR_SSL_UNKNOWN_CIPHER                                         \
    -0x7300 /**< An unknown cipher was received. */
...
#define MBEDTLS_ERR_SSL_CERTIFICATE_TOO_LARGE                                  \
    -0x7500 /**< Our own certificate(s) is/are too large to send in an SSL     \
               message. */
...
#define MBEDTLS_ERR_SSL_PEER_VERIFY_FAILED                                     \
    -0x7800 /**< Verification of our peer failed. */
...
#define MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE_REQUEST                             \
    -0x7A80 /**< Processing of the CertificateRequest handshake message        \
               failed. */
...
#define MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP                          \
    -0x7C80 /**< Processing of the ClientKeyExchange handshake message failed  \
               in DHM / ECDH Read Public. */
#define MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_CS                          \
    -0x7D00 /**< Processing of the ClientKeyExchange handshake message failed  \
               in DHM / ECDH Calculate Secret. */
...
#define MBEDTLS_ERR_SSL_NO_USABLE_CIPHERSUITE                                  \
    -0x6980 /**< None of the common ciphersuites is usable (eg, no suitable    \
               certificate, see debug messages). */
#define MBEDTLS_ERR_SSL_WANT_READ                                              \
    -0x6900 /**< Connection requires a read call. */
...
#define MBEDTLS_SSL_DEFAULT_TICKET_LIFETIME                                    \
    86400 /**< Lifetime of session tickets (if enabled) */
#endif
...
struct mbedtls_ssl_config
{
    ...
    int (*f_cookie_check)(
        void *, const unsigned char *, size_t, const unsigned char *, size_t);
    void *p_cookie; /*!< context for the cookie callbacks   */
#endif

#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_SRV_C)
    /** Callback to create & write a session ticket                         */
    int (*f_ticket_write)(void *,
...
struct mbedtls_ssl_context
{
    ...
    mbedtls_ssl_transform
        *transform_negotiate; /*!<  transform params in negotiation */
 ...
 */
    ...
    char own_verify_data[MBEDTLS_SSL_VERIFY_DATA_MAX_LEN]; /*!<  previous
                                                              handshake verify
                                                              data */
    char peer_verify_data[MBEDTLS_SSL_VERIFY_DATA_MAX_LEN]; /*!<  previous
                                                               handshake verify
                                                               data */
#endif
};
...
extern int (*mbedtls_ssl_hw_record_init)(mbedtls_ssl_context *ssl,
                                         ...
                                         size_t maclen);
extern int (*mbedtls_ssl_hw_record_activate)(mbedtls_ssl_context *ssl,
                                             int direction);
 ...
 *                        mbedtls_ssl_get_verify_result() can be called after
 * the
 ...
 * mbedtls_ssl_get_verify_result() at
 ...
 *                 (void *parameter, const mbedtls_ssl_session *session). The
 * function
 *                 should create a cache entry for future retrieval based on
 *                 the data in the session structure and should keep in mind
 *                 that the mbedtls_ssl_session object presented (and all its
 * referenced
 ...
 *                      MBEDTLS_SSL_MINOR_VERSION_1 and
 ...
 *                 following parameters: (void *parameter, mbedtls_ssl_context
 * *ssl,
 ...
 *                 (Default: MBEDTLS_SSL_MAX_MAJOR_VERSION,
 ...
 *
 * \param conf     SSL configuration
 * \param use_tickets   Enable or disable (MBEDTLS_SSL_SESSION_TICKETS_ENABLED
 * or
 ...
 *
 * \param conf    SSL configuration
 * \param renegotiation     Enable or disable (MBEDTLS_SSL_RENEGOTIATION_ENABLED
 * or
 ...
 *                 HelloRequest is retransmited every time mbedtls_ssl_read()
 * times
 ...
 *                 place to write it, which only happens during
 * mbedtls_ssl_read().
 *
 * \param conf     SSL configuration
 * \param max_records Use MBEDTLS_SSL_RENEGOTIATION_NOT_ENFORCED if you don't
 * want to
 ...
 *                  (MBEDTLS_SSL_ALERT_LEVEL_WARNING or
