...
tt_result_t tt_zlibinf_create(IN tt_zlibinf_t *zli,
                              IN OPT tt_zlibinf_attr_t *attr)
{
    ...
    TT_ZSTREAM_INIT(&zli->zs);

    z_err = inflateInit2(&zli->zs, attr->window_bits); // zlib
    if (z_err != Z_OK) {
        TT_ERROR("fail to init zlib inflate");
        return TT_FAIL;
    }
