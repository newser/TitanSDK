...
void tt_x509crt_init(IN tt_x509crt_t *x)
{
    TT_ASSERT(x != NULL);
...
tt_result_t tt_x509crt_add(IN tt_x509crt_t *x, IN tt_u8_t *buf, IN tt_u32_t len)
{
    int n;
...
tt_result_t tt_x509crt_add_file(IN tt_x509crt_t *x, IN const tt_char_t *path)
{
    tt_buf_t buf;
    int n;
...
void tt_x509crt_destroy(IN tt_x509crt_t *x)
{
    TT_ASSERT(x != NULL);
...
tt_result_t tt_x509crt_verify(IN tt_x509crt_t *x,
                              IN tt_x509crt_t *ca,
                              IN OPT void *crl,
...
tt_u32_t tt_x509crt_dump_verify_status(IN tt_u32_t status,
                                       IN tt_char_t *buf,
                                       IN tt_u32_t len)
{
    TT_ASSERT(buf != NULL);
...
tt_u32_t tt_x509crt_dump(IN tt_x509crt_t *x, IN tt_char_t *buf, IN tt_u32_t len)
{
    TT_ASSERT(buf != NULL);
