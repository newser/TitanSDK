...
TT_TEST_ROUTINE_DECLARE(case_hash_cpp)
...
TT_TEST_CASE("case_hash", "testing hash algorithms", case_hash, NULL, NULL,
             NULL, NULL, NULL)
,

    TT_TEST_CASE("case_hash_cpp", "testing hash algorithms", case_hash_cpp,
                 NULL, NULL, NULL, NULL, NULL),

    TT_TEST_CASE("case_map_basic", "testing map basic", case_map_basic, NULL,
                 NULL, NULL, NULL, NULL),

    TT_TEST_CASE("case_map_share_key", "testing map, sharing key",
                 case_map_share_key, NULL, NULL, NULL, NULL, NULL),

    TT_TEST_CASE("case_ptrmap", "testing ptrmap", case_ptrmap, NULL, NULL, NULL,
                 NULL, NULL),
...
void __h1_node2key(IN tt_hnode_t *node, OUT const tt_u8_t **key,
...
TT_TEST_ROUTINE_DEFINE(case_hash)
{
    ...
    for (i = 0; i < sizeof(__h1_murmur3) / sizeof(__h1_murmur3[0]); ++i) {
        ...
        hc = tt_hash_murmur3((tt_u8_t *)__h1_murmur3[i].key,
                             __h1_murmur3[i].key_len, &hctx);
        TT_UT_EQUAL(hc, __h1_murmur3[i].hash, "");
    }

    // performace: murmur3
    num = (tt_u32_t *)tt_malloc(sizeof(tt_u32_t) * __TN_NUM);
    hc = (tt_u32_t *)tt_malloc(sizeof(tt_u32_t) * __HC_NUM);
...
TT_TEST_ROUTINE_DEFINE(case_hash_cpp)
{
    // tt_u32_t param = TT_TEST_ROUTINE_PARAM(tt_u32_t);
    tt_u32_t i;
    tt_hashctx_t hctx;
    ...
    start = tt_time_ref();
    for (i = 0; i < __TN_NUM; ++i) {
        uint32_t c = h2.calc((tt_u8_t *)&num[i], 4);
        hc[c % __HC_NUM] += 1;
    }
    end = tt_time_ref();
    t = (tt_u32_t)tt_time_ref2ms(end - start);
...
void __h2_node2key(IN tt_hnode_t *node, OUT const tt_u8_t **key,
                   OUT tt_u32_t *key_len)
{
...
tt_bool_t __hm_count(IN tt_u8_t *key, IN tt_u32_t key_len, IN tt_hnode_t *mnode,
                     IN void *param)
{
...
TT_TEST_ROUTINE_DEFINE(case_map_basic)
{
    ...
    for (i = 0; i < __h2_num; ++i) {
        ...
        for (j = 0; j < len; ++j) { __h2_cases[i].key[j] = (tt_u8_t)rand(); }
        ...
        // insert
        ret = tt_hmap_add(&hmap, __h2_cases[i].key, __h2_cases[i].key_len,
                          &__h2_cases[i].hnode);
        TT_UT_SUCCESS(ret, "");
        ...
        if (i % 100 == 0) {
            ...
            TT_UT_EQUAL(tt_hmap_contain_key(&hmap, __h2_cases[i].key,
                                            __h2_cases[i].key_len),
                        TT_TRUE, "");
            TT_UT_EQUAL(tt_hmap_contain(&hmap, &__h2_cases[i].hnode), TT_TRUE,
                        "");
    ...
    // iter hmap with lots of nodes
    {
        ...
        tt_hmap_iter(&hmap, &hi);
        while ((hn = tt_hmap_iter_next(&hi)) != NULL) { ++n; }
    ...
    for (i = 0; i < __h2_num; ++i) { tt_free(__h2_cases[i].key); }

    {
        tt_u32_t key[100] = {0};
        for (i = 0; i < 100; ++i) {
            tt_hnode_t *hn = (tt_hnode_t *)tt_malloc(sizeof(tt_hnode_t));
            TT_UT_NOT_NULL(hn, "");
            tt_hnode_init(hn);
            key[i] = tt_rand_u32();
...
#define __knum 1000
    ...
    for (i = 0; i < __knum; ++i) {
        ...
        // insert
        ret = tt_hmap_add(&hmap, (tt_u8_t *)&key[i], sizeof(tt_u32_t),
                          &__h2_cases[i].hnode);
        TT_UT_SUCCESS(ret, "");
        if (i % 100 == 0) {
            tt_hnode_t *node = NULL;
            ...
            TT_UT_EQUAL(tt_hmap_count(&hmap), i + 1, "");
            TT_UT_EQUAL(tt_hmap_empty(&hmap), TT_FALSE, "");
            TT_UT_EQUAL(tt_hmap_contain_key(&hmap, (tt_u8_t *)&key[i],
                                            sizeof(tt_u32_t)),
                        TT_TRUE, "");
            TT_UT_EQUAL(tt_hmap_contain(&hmap, &__h2_cases[i].hnode), TT_TRUE,
                        "");
...
tt_bool_t __pm_count(IN tt_u8_t *key, IN tt_u32_t key_len, IN tt_ptr_t p,
                     IN void *param)
{
    tt_u32_t *pi = (tt_u32_t *)param;
    *pi += 1;
...
TT_TEST_ROUTINE_DEFINE(case_ptrmap)
{
    ...
    for (i = 0; i < __h2_num; ++i) {
        ...
        if (i % 91 == 0) {
            TT_UT_EQUAL(tt_ptrhmap_contain_ptr(&hmap, &__h2_cases[i]), TT_FALSE,
                        "");
            TT_UT_EQUAL(tt_ptrhmap_contain_key(&hmap, __h2_cases[i].key,
                                               __h2_cases[i].key_len),
                        TT_FALSE, "");
        }

        // insert
        ret = tt_ptrhmap_add(&hmap, __h2_cases[i].key, __h2_cases[i].key_len,
                             &__h2_cases[i]);
        TT_UT_SUCCESS(ret, "");
        if (i % 100 == 0) {
            ...
            TT_UT_EQUAL(tt_ptrhmap_contain_key(&hmap, __h2_cases[i].key,
                                               __h2_cases[i].key_len),
                        TT_TRUE, "");
            TT_UT_EQUAL(tt_ptrhmap_contain_ptr(&hmap, &__h2_cases[i]), TT_TRUE,
                        "");

            p = (struct __h2_case_t *)tt_ptrhmap_find(&hmap, __h2_cases[i].key,
                                                      __h2_cases[i].key_len);
            TT_UT_NOT_EQUAL(p, NULL, "");
            TT_UT_EQUAL(p - &__h2_cases[0], i, "");

            // adding a duplicate key would fail
            ret = tt_ptrhmap_add(&hmap, __h2_cases[i].key,
                                 __h2_cases[i].key_len, &__h2_cases[i]);
            TT_UT_FAIL(ret, "");
        }
    }
    ...
    for (i = 0; i < __h2_num; ++i) {
        tt_hnode_t *hnode =
            (tt_hnode_t *)tt_ptrhmap_find(&hmap, __h2_cases[i].key,
                                          __h2_cases[i].key_len);
    ...
    // remove
    for (i = 0; (i < __h2_num) && (i % 10 == 0); ++i) {
        tt_bool_t b = tt_ptrhmap_remove_key(&hmap, __h2_cases[i].key,
                                            __h2_cases[i].key_len);
    ...
    for (i = 1; (i < __h2_num) && (i % 9 == 0); ++i) {
        tt_bool_t b =
            tt_ptrhmap_remove_pair(&hmap, __h2_cases[i].key,
                                   __h2_cases[i].key_len, &__h2_cases[i]);
        TT_UT_EQUAL(b, TT_TRUE, "");
        TT_UT_EQUAL(tt_ptrhmap_contain_key(&hmap, __h2_cases[i].key,
                                           __h2_cases[i].key_len),
    ...
    for (i = 1; (i < __h2_num) && (i % 13 == 0); ++i) {
        // &__h2_cases[i] => &ret
        tt_bool_t b = tt_ptrhmap_replace(&hmap, __h2_cases[i].key,
                                         __h2_cases[i].key_len, &ret);
        TT_UT_EQUAL(b, TT_TRUE, "");
        TT_UT_EQUAL(tt_ptrhmap_contain_ptr(&hmap, &ret), TT_FALSE, "");
        TT_UT_EQUAL(tt_ptrhmap_find(&hmap, __h2_cases[i].key,
                                    __h2_cases[i].key_len),
                    &ret, "");
        TT_UT_EQUAL(tt_ptrhmap_contain_ptr(&hmap, &__h2_cases[i]), TT_FALSE,
                    "");
        ...
        // &ret => &__h2_cases[i]
        b = tt_ptrhmap_replace_equal(&hmap, __h2_cases[i].key,
                                     __h2_cases[i].key_len, &ret,
                                     &__h2_cases[i]);
        TT_UT_EQUAL(b, TT_TRUE, "");
        TT_UT_EQUAL(tt_ptrhmap_find(&hmap, __h2_cases[i].key,
