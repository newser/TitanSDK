...
tt_result_t tt_ssh_string_render(IN tt_buf_t *buf, IN tt_u8_t *string_val,
                                 IN tt_u32_t string_len)
{
    TT_DO(tt_buf_put_u32_n(buf, string_len));

    if (string_len > 0) { TT_DO(tt_buf_put(buf, string_val, string_len)); }

    return TT_SUCCESS;
}

tt_result_t tt_ssh_mpint_render(IN tt_buf_t *buf, IN tt_u8_t *mpint,
                                IN tt_u32_t mpint_len, IN tt_bool_t negative)
{
    ...
    if (mpint_len > 0) {
        if (h1 != 0) { TT_DO(tt_buf_put_u8(buf, 0)); }
...

{
    ...
    if (name_num > 0) { len += (tt_u32_t)tt_strlen(name[0]); }
...
tt_result_t tt_ssh_namelist_render(IN tt_buf_t *buf, IN const tt_char_t *name[],
                                   IN tt_u32_t name_num)
{
    tt_u32_t len = 0, i;
    ...
    if (name_num > 0) {
        ...
        if (n > 0) { TT_DO(tt_buf_put(buf, (tt_u8_t *)name[0], n)); }
    }
...
tt_u8_t tt_sshmsg_padlen(IN tt_u32_t data_len, IN tt_u32_t block_size,
                         IN tt_u32_t pad_block, IN tt_u32_t max_pad_block)
{
    tt_u32_t padlen;
    tt_u32_t n;
    ...
    if (max_pad_block == 0) { max_pad_block = 2; }
